...
def create_access_token(data: dict):
    to_encodes = data.copy()

    expire = datetime.utcnow() + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    #the to_encode is also a libary in it's own right 
    to_encodes.update({"exp": expire})

    encoded_jwt = jwt.encode(to_encodes, SECRET_KEY, algorithm=ALGORITHM)
